...
class Buku extends CI_Controller
{
    ...
    /**
     * Method to handle the request for the kategori page
     *
     * This method is responsible for loading the data required for the kategori page.
     * It also handles the form submission for adding a new kategori.
     *
     * @return void
     */
    public function kategori()
    {
        // Set the judul (title) of the page
        $data['judul'] = 'Kategori Buku';

        // Fetch the user data from the session
        $data['user'] = $this->ModelUser->cekData(['email' => $this->session->userdata('email')])->row_array();

        // Fetch the kategori data from the database
        $data['kategori'] = $this->ModelBuku->getKategori()->result_array();
        ...
        if ($this->form_validation->run() == false) {
            // Load the header, sidebar, topbar and the kategori view
            $this->load->view('templates/header', $data);
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            ...
            // If the form validation passes, create a new kategori
            $data = [
                'kategori' => $this->input->post('kategori')
            ];

            // Call the ModelBuku's simpanKategori method to save the new kategori
            $this->ModelBuku->simpanKategori($data);

            // Redirect the user to the kategori page
